...
<div class="container" style="margin: 80px auto;">
    <div class="col-xs-12" style="height: 700px;">
        <div style="margin: 20px 0;">
            <ul>
                <c:forEach items="${translate_pageList}" var="translate_pageList">
                    <li style="margin-bottom: 20px;background-color:#f7f7f7;padding:10px 10px;">
					  	    <span>
					  		    <p style="font-size: 20px;">${translate_pageList.translate_pageName}</p>
					  	    </span>
                        <span>摘要: ${translate_pageList.translate_pageSummary }....</span>
                        <br>
                        <br>
					  	    <span>
                                <a href="/user/translate/TranslateExam.do?translate_pageId=${translate_pageList.translate_pageId}">开始训练</a>&nbsp;&nbsp;
					  	    </span>
                    </li>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#pre').attr('href','/user/translate/listTranslate_Page.do?page='+pre)
        for (var i = 0; i < pageTotal; i++) {
            ...
            $('#current'+(i+1)).attr('href','/user/translate/listTranslate_Page.do?page='+(i+1))
            if ((i+1)==currentPage) {
        ...
        $('#next').attr('href','/user/translate/listTranslate_Page.do?page='+next);
        if (pageTotal==1){
